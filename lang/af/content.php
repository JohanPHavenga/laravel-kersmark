<?php
return [
    'kennisgewing_img' => 'kersmark-kennisgewing-afr.jpg',
    'menu' => [
        'home' => 'Tuis',
        'vendor_application' => 'Uitstallers Aansoek',
        'contact' => 'Kontak',
    ],
    'heading' => [
        'h1' => "Uitsig Kersmark",
        'date' => "17 November - 6 Desember 2025",
        'time' => "Mon to Vry 10:00-19:00<br>Sat 09:00-16:00",
        'intro1' => "Welkom by ons 25ste Kersmark! Ons mark is uniek agv ons vriendelike atmosfeer en hoofsaaklik handgemaakte produkte van plaaslike enntreprenuers!<br />Toegang is gratis tot ons ontspanne en verwelkomede mark wat rolstoelvriendelik is. Besoekers sal getrakteer word met n goeie verskeidenheid uitstallers en heerlike versnapperinge en maaltye by die opwip restaurant.",
        'intro2' => "Daar is 'n sentrale betaalpunt met verskeie kasregisters by die uitgang. Die publiek kan per kredietkaart, debietkaart, snapscan of kontant betaal.",
    ],
    'contact' => [
        'heading' => 'Kontak Besonderhede',
        'address' => '<strong>NG Kerk Bellville-Uitsig</strong><br>
        17 Mountainviewrylaan<br>
        Ridgeworth<br>
        Bellville',
        'phone_title' => 'Telefoon Nommer',
        'phone_text' => 'Telefoon',
        'email_title' => 'Epos Adres',
        'email_text' => 'Epos',
    ],
    'vendor' => [
        'heading' => 'Uitstallers Aansoek',
        'step1' => [
            'heading' => 'Stap 1: Uitstallers Information',
            'date' => [
                'heading' => 'Datum',
                'text' => 'Maandag 17 November to Saterdag 6 Desember 2025',
            ],
            'where' => [
                'heading' => 'Waar',
                'text' => 'NG Kerk Bellville-Uitsig, h/v Mountainviewrylaan en Ridgeworthrylaan, Ridgeworth, Bellville',
            ],
            'hours' => [
                'heading' => 'Besigheidsure',
                'text' => '<p>Maandae tot Vrydae van 10:00 tot 19:00<br>Saterdae van 09:00 tot 16:00.</p>',
            ],
            'fees' => [
                'heading' => 'Fooie',
                'text' => '<p>Die registrasiefooi vir die geselekteerde uitstallers is:</p>
                <ul style="margin-bottom: 16px">
                <li><b>R1050</b> per volle tafel (+ 180cm x 75cm)</li>
                <li><b>R550</b> per halwe tafel (+ 90cm x 75cm)</li>
                <li><b>R350</b> per 1/3 tafel (60cm x 75cm)</li>
                <li><b>R1200</b> - 4 Vlak Hout rak deur ons verskaf (ongeveer 90cm x 75cm)</li>
                </ul>
                <ul>
                <li>Betaalbaar voor of op 31 Augustus 2025</li>
                <li>Tafelfooie sluit toesig in. Deposito\'s is nie terugbetaalbaar nie. Geen laat betalings sal aanvaar word nie</li>
                <li>20% van alle verkope word aan die Kerk geskenk. (Slegs 17% van verkope op silver/goued juweliersware)</li>
                <li>Jy moet dus jou artikels merk sodat die 20% reeds ingesluit is. Geen artikelpryse mag na die keuringsdag of tydens die mark verhoog of verlaag word nie. Rond asseblief jou verkoopprys af tot die naaste R1</li>
                </ul>',
            ],
            'selection' => [
                'heading' => 'Keuring',
                'text' => '<p>Weens oorweldigende belangstelling, doen meer uitstallers aansoek as wat ons kan akkommodeer.<br>
                As gevolg hiervan, word \'n keurpaneel vir die seleksie van produkte aangestel. Die besluite van die paneel is finaal.<br>
                Die aansoekproses is soos volg:</p>
                <ol>
                <li>Gaan na www.kersmark.co.za  en voltooi die aansoekvorm tesame met ten minste 6 goeie gehalte en duidelike foto\'s van jou produkte en tafeluitleg</li>
                <li>Jou aansoek sal hersien word en indien nodig kan \'n verkoper gevra word om hul produk te kom wys. Hier sal ons fokus op die verpakking, kwaliteit en uitstalling van jou produk</li>
                <li>Sodra aansoeke gesluit is, sal die keuring proses begin en die uitstaller sal nie later as <b>31 Julie 2025</b> in kennis gestel word of hul aansoek suksesvol was</li>
                </ol>
                <p>Seleksiekriteria wat oorweeg sal word:</p>
                <ul>
                <li>handgemaakte produkte sal voorkeur geniet</li>
                <li>aanbieding van produkte</li>
                <li>kwaliteit van produkte</li>
                <li>bekostigbaarheid van produkte</li>
                </ul>',
            ],
            'schedule' => [
                'heading' => 'Tyd Skedule',
                'text' => '<p><b>13 Junie </b> - Sperdatum vir aansoeke<br>
                <b>31 Julie</b> - Kennisgewing van wie gekies is<br>
                <b>31 Augustus</b> - Sperdatum vir betaling van tafelfooie<br>
                <b>13 &amp; 14 November</b> - 09:00-19:00 Saal oop vir uitpak<br>
                <b>17 November</b> - Mark begin om 10:00<br>
                <b>06 Desember</b> - Mark eindig om 16:00<br>
                <b>06 Desember</b> - 17:00 - 19:00 Op pak en verwydering van alle uitstallings en produkte<br>
                <b>23 Desember</b> - Laaste datum vir elektroniese betaling aan uitstallers
                </p>',
            ],
            'info' => [
                'heading' => 'Algemene Inligting',
                'text' => '<ul>
                <li><b>Daaglikse verkope opdatering:</b><br>Hierdie fasiliteit is beskikbaar tydens die mark. Meer inligting sal later volg</li>
                <li><b>Uitpak:</b><br>Die saal sal Donderdag 13 November en Vrydag 14 November vanaf 09:00 - 19:00 oop wees vir uitpak. Jy is verantwoordelik vir die dra, uitpak en vertoon van jou produkte.  Ekstra voorraad moet onder jou tafel gestoor word. Ons verantwoordelikheid ten opsigte van sekuriteit begin Vrydag 14 November om 18:00</li>
                <li><b>Oppak:</b><br>Die Mark eindig op Saterdag 6 Desember om 16:00.<br>Die saal sal van 17:00 - 19:00 oop wees vir die oppak en wegneem van alle uitstallings en produkte</li>
                <li><b>Sosiale Media:</b><br>Die mark sal wyd bemark word op facebook, Instagram etc.<br>Daar sal van gekeurde uitstallers verwag word om ons te voorsien van n paar produkte fotos vir bemarking doeleindes</li>
                </ul>',
            ],
            'terms' => [
                'heading' => 'Bepalings en Voorwaardes',
                'text' => '<ul>
                <li>Jy moet genoeg voorraad vir die hele tydperk hê. ’n Maksimum boete van 30% van jou omset sal verhaal word as jou voorraad opraak</li>
                <li>Die feit dat jy aan ander markte deelneem, mag nie jou voorraadvlakke hier raak nie en ook nie jou pryse nie - as jy vir minder by ander markte verkoop het ons die reg om jou produkte te verwyder</li>
                <li>Pryse mag nie na die keuringsdag of tydens die mark verander word nie</li>
                <li>Slegs geselekteerde items mag uitgestal word, dit wil sê wat die komitee op jou produklys gekies het</li>
                <li>Alle voedsel moet die volgende bevat: Vervaldatum, bestandele inhoud, belangrike inligting rakende die berging van die produk, bv. op \'n rak of in \'n yskas (voor of na oopmaak) en adres waar produkte gemaak word.<br>(Verkopers met kosstalletjies is verantwoordelik vir die skoonmaak en vul van hul proebakkies.)</li>
                <li>Verkopers van juweliersware (silwer en goud) moet daagliks teenwoordig wees</li>
                <li>NG Kerk Bellville-Uitsig, die koördineerders en komitee tree bloot as fasiliteerder tydens die mark op en is nie verantwoordelik vir enige belastingnakoming of verpligting van uitstallers nie. Vir jou deelname maak jy \'n bydrae tot Bellville-Uitsig se gemeentefonds</li>
                <li>Die koördineerders en komitee is nie verantwoordelik vir enige verliese wat \'n verkoper mag hê vir die duur van die mark nie</li>
                <li>Die koördineerders en komitee van die Uitsig Kersmark distansieer hulle van enige dispute en verskille tussen uit</li>
                <li>Streng veiligheidsmaatreëls word tydens die mark getref. Toegang word te alle tye beheer. Verskeie videokameras word vir monitering gebruik. Hou asseblief by die toegekende  tye vir in- en uitpak aangesien die sekuriteitspersoneel streng daarvolgens werk</li>
                <li>By voorbaat dankie vir jou samewerking in hierdie verband. Die veiligheid en sekuriteit van ons uitstallers en hul uitstallings is vir ons baie belangrik, daarom sal geen uitsondering gemaak word nie. Voorsorgmaatreëls ten opsigte van skade of verlies sal sover moontlik deur die reëlingskomitee getref word. Bellville Uitsig-gemeente en die reëlingskomitee is egter nie verantwoordelik vir enige skade of verlies wat gely word nie</li>
                </ul>',
            ],
        ],
        'step2' => [
            'heading' => 'Stap 2: Voltooi Aansoek Form',
            'text' => 'Sal binnekort beskikbaar wees.',
            'button_vendor_app' => 'Uitstallers Aansoek<span>Doen aansoek om \'n uitstaller te wees</span>'
        ],
    ],

];
